const getAskAIRoot = (target: EventTarget | null) => {
  return target instanceof Element ? target.closest<HTMLElement>(".ask-ai") : null
}

const getAskAIElements = (root: HTMLElement) => {
  ...
  if (!toggle || !panel || !form || !input || !answer) return null

  return { toggle, panel, form, input, answer }
}

document.addEventListener("click", (event) => {
  const target = event.target
  if (!(target instanceof Element) || !target.closest(".ask-ai-toggle")) return

  const root = getAskAIRoot(target)
  if (!root) return

  const elements = getAskAIElements(root)
  if (!elements) return

  const { toggle, panel, input } = elements
...
document.addEventListener("submit", async (event) => {
  const target = event.target
  if (!(target instanceof HTMLFormElement) || !target.classList.contains("ask-ai-form")) return
